// World Political GameMap - 2D Edition for Unity - Main Script
...
//					labelsShadowMaterial.renderQueue--;
	...
	#region Public API area
        ...
        /// <summary>
		/// Returns the centre vector of a country in the countries collection by its name.
		/// </summary>
        public Vector2 GetCountryCentreVector(string countryName) {
            if (countryLookup.ContainsKey(countryName))
                return countries[countryLookup[countryName]].center;
            else
                throw new System.InvalidOperationException("There is not the country name");
                
        }
        
        public bool SetCountryLabel(string countryName, string text) {
	        if (countryLookup.ContainsKey(countryName)) {
		        var country = countries[countryLookup[countryName]];
		        country.customLabel = text;
		        return true;
	        }
	        else {
		        return false;
		...
		/// <param name="localPosition">GameMap coordinates in the range of (-0.5 .. 0.5)</param>
		...
		/// <summary>
		/// Adds a new country which has been properly initialized. Used by the GameMap Editor. Name must be unique.
		/// </summary>
