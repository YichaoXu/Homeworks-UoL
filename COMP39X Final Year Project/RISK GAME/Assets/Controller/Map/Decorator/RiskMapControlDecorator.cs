...
{
    public class ColorizedMapDecorator : AbstractMapDecorator {
        ...
        public ColorizedMapDecorator(AbstractMapDecorator previous) : base(previous){
            continentColorSetting = DEFAULT_CONTINENT_COLOR_SETTING;
            ColorizeAllContinent();
        }

        public ColorizedMapDecorator(WorldMap2D map): base(map) {
    ...
    public class NaturalMapDecorator : AbstractMapDecorator {

        public NaturalMapDecorator(AbstractMapDecorator previous) : base(previous) {
            mapView.HideCountrySurfaces();
        }

        public NaturalMapDecorator(WorldMap2D mapView): base(mapView) {
